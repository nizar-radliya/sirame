...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>
			Verifikasi Pelaporan PKKPR
			<small><?= webname() ?></small>
		</h1>
		<ol class="breadcrumb">
			<li><a href="<?php echo base_url(); ?>pelaporan-adm"><i class="fa fa-calendar-check-o"></i>Pelaporan PKKPR</li></a></li>
			<li><a href="<?php echo base_url(); ?>pelaporan-adm/detil/<?= $kkpr[0]->nomor ?>"><i class="fa fa-binoculars"></i>Data</li></a></li>
			<li class="active"><i class="fa fa-edit"></i> Verifikasi</li>
		</ol>
	</section>
	<!-- Main content -->
				...
				<div class="box box-success" style="margin-bottom: 0">
					<div class="box-body">
						<div class="col-sm-12" style="padding: 0; overflow-x: scroll; overflow-y: hidden;">
							...
							<form role="form" method="post" enctype="multipart/form-data" action="<?php echo base_url(); ?>pelaporan-adm/edit">
								...
								<table class="table table-bordered" style="margin-bottom: 0">
										...
										<td width="50%">
											...
											<select class="form-control" name="jenisdok" required>
												...
												<?php foreach ($jenisdok as $i) { ?>
													<?php if ($i == $pelaporan[0]->jenisdok) { ?>
														<option value="<?php echo $i; ?>" selected><?php echo $i; ?></option>
													<?php } else { ?>
														<option value="<?php echo $i; ?>"><?php echo $i; ?></option>
													<?php } } ?>
											</select>
											<input type="hidden" name="nomor" value="<?= $pelaporan[0]->nomor ?>">
											<input type="hidden" name="idpemohon" value="<?= $pelaporan[0]->idpemohon ?>">
											<input type="hidden" name="idpelaporan" value="<?= $pelaporan[0]->idpelaporan ?>">
										</td>
										<td width="50%">
											<label>Hasil Scan Dokumen</label>
											<input type="file" id="exampleInputFile" name="filepelaporan">
											<a href="<?php echo base_url(); ?>pelaporan-adm/unduh/?file=<?= $pelaporan[0]->filepelaporan; ?>" target="_blank">
												<button type="button" class="btn bg-purple-gradient btn-flat margin" title="Unduh" style="margin: 5px 0 0 0; min-width: 40px">
									...
									<tr>
										<td>
											...
											<input type="text" class="form-control" name="nomordok" value="<?= $pelaporan[0]->nomordok ?>" required>
										</td>
										<td>
											<label>Tanggal Dokumen</label>
											<input type="date" class="form-control" name="tgldok" value="<?= $pelaporan[0]->tgldok ?>" required>
										</td>
									</tr>
									<tr>
										<td>
											<label>Luas Tanah m2 (sesuai dokumen)</label>
											<?php $luasmax = $kkpr[0]->luasdisetujui-$luaspelaporan; ?>
											<input type="number" class="form-control" name="luasrealisasi" value="<?= $pelaporan[0]->luasrealisasi ?>" max="<?= $luasmax ?>" required>
										</td>
										<td>
											<label>Nama Pejabat (sesuai dokumen)</label>
											<input type="text" class="form-control" name="pejabat" value="<?= $pelaporan[0]->pejabat ?>" required>
										</td>
									</tr>
									</tr>
									<tr>
										<td>
											...
											<textarea id="editor1" name="catatan" required><?= $pelaporan[0]->catatan ?></textarea>
										</td>
										<td>
											<label>Sketsa Lokasi</label>
											<div id="map" style="padding: 0; height: 440px; width: 100%; border: 2px solid #C2C2C2; border-radius: 5px; margin: 0 auto"> <!-- ini id="map" bisa di ganti dengan nama yang di inginkan -->
									...
									<tr>
										<td colspan="2">
											...
											<select class="form-control" name="statuslaporan" required>
												<?php foreach ($statuslaporan as $i) { ?>
													<?php if ($i == $pelaporan[0]->statuslaporan) { ?>
														<option value="<?php echo $i; ?>" selected><?php echo $i; ?></option>
													<?php } else { ?>
														<option value="<?php echo $i; ?>"><?php echo $i; ?></option>
													<?php } } ?>
