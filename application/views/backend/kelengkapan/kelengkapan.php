...
<div class="content-wrapper">
	...
	<section class="content-header">
		...
		<ol class="breadcrumb">
			<li><a href="<?php echo base_url(); ?>kkpr"><i class="fa fa-file-text-o"></i>KKPR</li></a></li>
			<li class="active"><i class="fa fa-check-square-o"></i> Kelengkapan KKPR</li>
		</ol>
	</section>
	<!-- Main content -->
				...
				<div class="box box-success" style="margin-bottom: 0">
					<div class="box-header with-border">
						...
						<select class="form-control select2" name="" id="iddi" style="width: auto" onchange="javascript:location.href = this.value;">
							<option disabled>Nomor KKPR</option>
							<?php foreach ($noreg as $field) { ?>
								<?php if ($field->nomor == $kkpr[0]->nomor) { ?>
									<option value="<?php echo base_url(); ?>kelengkapan-adm/index/<?= $field->nomor; ?>" selected><?= $field->nomor; ?></option>
								<?php } else { ?>
									<option value="<?php echo base_url(); ?>kelengkapan-adm/index/<?= $field->nomor; ?>"><?= $field->nomor; ?></option>
								<?php } } ?>
						</select>

						<?php if (menu(32) && ($kkpr[0]->statuskkpr != 'KKPR VALID')) { ?>
							<a href="#" class="setujui-semua" data-id="<?= $kkpr[0]->nomor; ?>">
								<button type="button" class="btn bg-green-gradient btn-flat margin pull-right"
					...
					<div class="box-body">
						<div class="col-sm-12" style="padding: 0; overflow-x: scroll; overflow-y: hidden;">
								...
								<?php $n=1; foreach ($jeniskelengkapan as $field) {
									...
									<tr>
										...
										<td style="width: auto">
											...
											<a href="<?php echo base_url(); ?>kelengkapan-adm/unduh/?file=<?= $kkpr_kelengkapan[0]->filekelengkapan ?>" target="_blank">
							...
							<table class="table table-bordered" style="text-align: justify; margin-bottom: 0">
								<tr>
									<th colspan="4" style="text-align: center; background-color: #0D340D; color: #ffffff;"><h4 class="text-bold">Data KKPR</h4></th>
								</tr>
								<tr>
									<td style="width: 25%"><label>Nomor KKPR</label></td>
...
<script>
	$(function(){
		...
		$(document).on('click','.kelengkapan-add',function(e){
			e.preventDefault();
			$("#modalAdd").modal('show');
			$.get('<?php echo base_url(); ?>kelengkapan-adm/add',
...
<script>
	$(function(){
		...
		$(document).on('click','.kelengkapan-del',function(e){
			e.preventDefault();
			$("#modalDel").modal('show');
			$.get('<?php echo base_url(); ?>kelengkapan-adm/kelengkapan-del',
					{id:$(this).attr('data-id'),id2:$(this).attr('data-id2'),data:csfrData},
					function(html){
...
<script>
	$(function(){
		...
		$(document).on('click','.kelengkapan-cek',function(e){
			e.preventDefault();
			$("#modalCek").modal('show');
			$.get('<?php echo base_url(); ?>kelengkapan-adm/kelengkapan-cek',
					{id:$(this).attr('data-id'),id2:$(this).attr('data-id2'),data:csfrData},
					function(html){
						$(".modal-body").html(html);
					}
...
<script>
	$(function(){
		...
		$(document).on('click','.setujui-semua',function(e){
			...
			$.get('<?php echo base_url(); ?>kelengkapan-adm/setujui-semua',
