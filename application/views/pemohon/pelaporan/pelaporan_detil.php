						...
						<select class="form-control select2" name="" id="iddi" style="width: auto" onchange="javascript:location.href = this.value;">
							...
							<?php foreach ($noreg as $field) { ?>
								<?php if ($field->nomor == $kkpr[0]->nomor) { ?>
									<option value="<?php echo base_url(); ?>pelaporan/detil/<?= $field->nomor; ?>" selected><?= $field->nomor; ?></option>
								<?php } else { ?>
									<option value="<?php echo base_url(); ?>pelaporan/detil/<?= $field->nomor; ?>"><?= $field->nomor; ?></option>
								<?php } } ?>
						</select>

						<a href="<?php echo base_url(); ?>pelaporan/form-add/<?= $kkpr[0]->nomor; ?>">
								...
								<?php $n=1; foreach ($pelaporan as $field) { ?>
									<tr>
										...
										<td>
											...
											<a href="<?php echo base_url(); ?>pelaporan/unduh/?file=<?= $field->filepelaporan; ?>" target="_blank">
											...
											<?php if ($field->statuslaporan != 'PELAPORAN DITERIMA') { ?>
												<a href="<?php echo base_url(); ?>pelaporan/form-edit/<?= $field->idpelaporan; ?>">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span></button>
				<h4 class="modal-title"><i class="icon fa fa-file-text-o"></i> Data Pelaporan PKKPR</h4>
			</div>
			<div class="modal-body">
			</div>
		</div>
...
<script>
	$(function(){
		...
		$(document).on('click','.skip',function(e){
			...
			$.get('<?php echo base_url(); ?>pelaporan/skip-pelaporan',
...
<script>
	$(function(){
		...
		$(document).on('click','.pengkajian',function(e){
			...
			$.get('<?php echo base_url(); ?>pelaporan/pengkajian-teknis',
					{id:$(this).attr('data-id'),data:csfrData},
					function(html){
						$(".modal-body").html(html);
					}
...
<script>
	$(function(){
		...
		$(document).on('click','.ket',function(e){
			...
			$.get('<?php echo base_url(); ?>pelaporan/catatan',
					{id:$(this).attr('data-id'),data:csfrData},
					function(html){
						$(".modal-body").html(html);
					}
...
<script>
	$(function(){
		...
		$(document).on('click','.del',function(e){
			...
			$.get('<?php echo base_url(); ?>pelaporan/form-del',
					{id:$(this).attr('data-id'),id2:$(this).attr('data-id2'),data:csfrData},
					function(html){
						$(".modal-body").html(html);
					}
