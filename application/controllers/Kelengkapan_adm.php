...
class Kelengkapan_adm extends CI_Controller {
    ...
    public function index()
	{
		...
		$data['menu'] 	= $this->menu;
		$data['view'] 	= 'backend/kelengkapan/kelengkapan';
	...
	public function add()
	{
		...
		$this->load->view('backend/kelengkapan/kelengkapan_add', $data);
	}
	...
	public function add_kelengkapan()
	{
		...
		if (!$this->upload->do_upload('filekelengkapan')) {
			...
			redirect('kelengkapan-adm/index/'.$nomor);
		} else {
	...
	public function kelengkapan_del()
	{
		...
		$this->load->view('backend/kelengkapan/kelengkapan_del', $data);
	}
	...
	public function del()
	{
		...
		if ($this->delete->delete_one($table, $by, $id2)) {
			...
			redirect('kelengkapan-adm/index/'.$id);
		} else {
	...
	public function kelengkapan_cek()
	{
		...
		$this->load->view('backend/kelengkapan/kelengkapan_cek', $data);
	}
	...
	public function tanda_terima()
	{
		...
		$html = $this->load->view('backend/kkpr/kkpr_tandaterima',$this->data, true);

		// run dompdf
		$this->pdfgenerator->generate($html, $file_pdf,$paper,$orientation);
	}

	public function setujui_semua()
	{
		permission(31);
		$data['id'] = $this->input->get('id');
		$this->load->view('backend/kelengkapan/kelengkapan_disetujui', $data);
	}
	...
	public function setujui()
	{
		...
		$by = 'nomor';
		$data = array(
			'statuskkpr' => 'PKKPR VALID',
		);

		if ($this->update->update_one($table, $by, $id, $data)) {
			$table = 'kkpr_kelengkapan';
			$data = array(
				'statuskelengkapan' => 'KELENGKAPAN DITERIMA',
				'tglverifikasi' => date('Y-m-d H:i:s'),
				'idpengguna' => $this->session->userdata('idpengguna')
			);
			$this->update->update_one($table, $by, $id, $data);

			$this->session->set_flashdata('success', 'Berhasil Simpan: semua kelengkapan sudah disetujui.');
			redirect('pelaporan-adm/detil/'.$id);
		} else {
			$this->session->set_flashdata('error', 'Gagal Simpan: data belum tersimpan ke server.');
			redirect('kelengkapan-adm/'.$id);
		}
	}
}
