...
L.Control.mouseCoordinate  = L.Control.extend({
    ...
    _update: function(e){
        ...
        if(this.options.gps){
            ...
            content += "<tr><td>GPS</td><td>:</td><td>" + dLat + "</td><td> " + dLng +"</td></tr>";
            if(this.options.gpsLong){
                var gpsMinuten = this._geo2geodeziminuten(gps);
                content += "<tr><td></td><td></td><td class='coords'>"+ gpsMinuten.NS + " " + gpsMinuten.latgrad + "&deg; "+ gpsMinuten.latminuten+"</td><td class='coords'> " + gpsMinuten.WE + " "+ gpsMinuten.lnggrad +"&deg; "+ gpsMinuten.lngminuten +"</td></tr>";
                var gpsMinutenSekunden = this._geo2gradminutensekunden(gps);
                content += "<tr><td></td><td></td><td>"+ gpsMinutenSekunden.NS + " " + gpsMinutenSekunden.latgrad + "&deg; "+ gpsMinutenSekunden.latminuten + "&prime; "+ gpsMinutenSekunden.latsekunden+"&Prime;</td><td> " + gpsMinutenSekunden.WE + " "+ gpsMinutenSekunden.lnggrad +"&deg; "+ gpsMinutenSekunden.lngminuten + "&prime; "+ gpsMinutenSekunden.lngsekunden+"&Prime;</td></tr>";
            }
        }
        if(this.options.utm){
            var utm = UTM.fromLatLng(gps);
            if(utm !== undefined){
                content += "<tr><td>UTM</td><td>:</td><td colspan='2'>"+utm.zone+"&nbsp;" +utm.x+"&nbsp;" +utm.y+"</td></tr>";
            }
        }
        if(this.options.utmref){
            var utmref = UTMREF.fromUTM(UTM.fromLatLng(gps));
            if(utmref !== undefined){
                content += "<tr><td>UTM REF</td><td>:</td><td colspan='2'>"+utmref.zone+"&nbsp;" +utmref.band+"&nbsp;" +utmref.x+"&nbsp;" +utmref.y+"</td></tr>";
            }
        }
        if(this.options.qth){
            var qth = QTH.fromLatLng(gps);
            content += "<tr><td>QTH</td><td>:</td><td colspan='2'>"+qth+"</td></tr>";
        }
        if(this.options.nac){
            var nac = NAC.fromLatLng(gps);
            content += "<tr><td>NAC</td><td>:</td><td colspan='2'>"+nac.y+" "+ nac.x +"</td></tr>";
        }
