...
function m_ujian_s() {
	...
	if (parseInt(form.jumlah_soal) > parseInt(form.jumlah_soal1)) {
		alert('Jumlah soal pada mata kuliah ini belum mencukupi..!');
	} else {
		$.ajax({		
			type: "POST",
			...
			if (data.status == "ok") {
				...
				if (jml_data > 0) {
					$.each(data.data, function(i, item) {
						if (item.ok == "1") {
							hate += '<label><input type="checkbox" value="'+item.id+'" name="id_mapel_'+item.id+'" checked> &nbsp;'+item.nama+'</label> <br />';
						} else {
							hate += '<label><input type="checkbox" value="'+item.id+'" name="id_mapel_'+item.id+'"> &nbsp;'+item.nama+'</label> <br />';
						}
					});				
				} else {
					hate += 'Belum ada data..';
...
function m_guru_u(id) {
	if (confirm('Anda yakin..? Username otomatis adalah "dosen'+id+'", dan Password otomatis adalah "dosen"')) {
		$.ajax({
			type: "GET",
...
function m_matkul_guru(id) {
	$.ajax({
		type: "GET",
		url: base_url+"adm/m_mapel/ambil_guru/"+id,
		success: function(data) {
			if (data.status == "ok") {
				var jml_data	= Object.keys(data.data).length;
				var hate 	= '<div class="modal fade" id="m_siswa_matkul" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"><div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 id="myModalLabel">Setting Dosen Pengajar</h4></div><div class="modal-body"><form name="f_siswa_matkul" id="f_siswa_matkul" method="post" onsubmit="return m_matkul_guru_s();"><input type="hidden" name="id_mhs" id="id_mhs" value="'+id+'"><div id="konfirmasi"></div>';
...
function m_matkul_guru_s() {
	var f_asal	= $("#f_siswa_matkul");
	var form	= getFormData(f_asal);

	$.ajax({
		type: "POST",
		url: base_url+"adm/m_mapel/simpan_guru",
		data: JSON.stringify(form),
		dataType: 'json',
		contentType: 'application/json; charset=utf-8'
	}).done(function(response) {
